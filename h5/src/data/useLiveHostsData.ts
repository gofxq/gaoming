...
export function useLiveHostsData(props: {
  ...
  useEffect(() => {
    ...
    stream.addEventListener("open", () => setStreamState("实时推送中"));
    stream.addEventListener("error", () => setStreamState("重连中"));
    ...
    stream.addEventListener("host_upsert", (event) => {
      ...
      startTransition(() => {
        setStreamEventCount((current) => current + 1);
        setAgents((current) => ({ ...current, [payload.item!.host_uid]: payload.item! }));
