...
func (s *Store) RegisterAgent(ctx context.Context, req contracts.RegisterAgentRequest, now time.Time) (state.HostSnapshot, contracts.AgentConfig, string, error) {
	...
	err := s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if !ok {
			return hostruntime.ErrHostNotFound
		}
		snapshot = item
		return nil
	})
...
func (s *Store) Heartbeat(ctx context.Context, req contracts.HeartbeatRequest, now time.Time) (state.HostSnapshot, contracts.AgentConfig, error) {
	...
	err := s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := s.updateAgentInstanceState(ctx, tx, hostID, req.AgentID, req.Seq, now); err != nil {
			return err
		}

		if err := s.upsertDigestStatus(ctx, tx, hostID, req.Digest, now); err != nil {
			return err
		}
...
func (s *Store) ReportMetrics(ctx context.Context, req contracts.PushMetricBatchRequest, digest contracts.AgentDigest, now time.Time) (state.HostSnapshot, error) {
	var snapshot state.HostSnapshot
	...
	err := s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := s.updateAgentInstanceState(ctx, tx, hostID, req.AgentID, req.BatchSeq, now); err != nil {
			return err
		}
		if err := s.upsertDigestStatus(ctx, tx, hostID, digest, now); err != nil {
			return err
		}
...
func (s *Store) CreateMaintenance(ctx context.Context, req contracts.CreateMaintenanceWindowRequest) (hostruntime.MaintenanceWindow, error) {
	...
	if err := s.db.WithContext(ctx).Create(&model).Error; err != nil {
		return hostruntime.MaintenanceWindow{}, err
	}

	return hostruntime.MaintenanceWindow{
...
func (s *Store) lookupAgentInstance(ctx context.Context, db *gorm.DB, hostUID string, agentID string) (int64, contracts.AgentConfig, error) {
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return 0, contracts.AgentConfig{}, hostruntime.ErrHostNotFound
		}
		return 0, contracts.AgentConfig{}, err
	}
...
func (s *Store) updateAgentInstanceState(ctx context.Context, db *gorm.DB, hostID int64, agentID string, seq int64, now time.Time) error {
	return db.WithContext(ctx).Model(&agentInstanceModel{}).
		Where(&agentInstanceModel{HostID: hostID, AgentID: agentID}).
		Updates(map[string]any{
			"state":        int(state.Up),
			"last_seen_at": now,
			"last_seq":     seq,
...
func (s *Store) upsertDigestStatus(ctx context.Context, db *gorm.DB, hostID int64, digest contracts.AgentDigest, now time.Time) error {
	var statusRow hostStatusCurrentModel
	err := firstOrNotFound(
		db.WithContext(ctx).Where(&hostStatusCurrentModel{HostID: hostID}),
		&statusRow,
	...
	statusRow.AgentState = int(state.Up)
	statusRow.OverallState = int(state.Up)
	statusRow.CPUUsagePct = digest.CPUUsagePct
	statusRow.MemUsedPct = digest.MemUsedPct
	statusRow.MemAvailableBytes = digest.MemAvailableBytes
	statusRow.SwapUsedPct = digest.SwapUsedPct
	statusRow.DiskUsedPct = digest.DiskUsedPct
	statusRow.DiskFreeBytes = digest.DiskFreeBytes
	statusRow.DiskInodesUsedPct = digest.DiskInodesUsedPct
	statusRow.DiskReadBPS = digest.DiskReadBPS
	statusRow.DiskWriteBPS = digest.DiskWriteBPS
	statusRow.DiskReadIOPS = digest.DiskReadIOPS
	statusRow.DiskWriteIOPS = digest.DiskWriteIOPS
	statusRow.Load1 = digest.Load1
	statusRow.NetRxBPS = digest.NetRxBPS
	statusRow.NetTxBPS = digest.NetTxBPS
	statusRow.NetRxPacketsPS = digest.NetRxPacketsPS
	statusRow.NetTxPacketsPS = digest.NetTxPacketsPS
	statusRow.LastAgentSeenAt = &now
