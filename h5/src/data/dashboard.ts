...
export const METRICS: MetricDefinition[] = [
  { key: "cpu_usage_pct", label: "CPU", unit: "%", source: (item) => Number(item.cpu_usage_pct || 0) },
  { key: "mem_used_pct", label: "内存", unit: "%", source: (item) => Number(item.mem_used_pct || 0) },
  { key: "mem_available_bytes", label: "可用内存", unit: "bytes", source: (item) => Number(item.mem_available_bytes || 0) },
  { key: "swap_used_pct", label: "Swap", unit: "%", source: (item) => Number(item.swap_used_pct || 0) },
  { key: "disk_used_pct", label: "磁盘", unit: "%", source: (item) => Number(item.disk_used_pct || 0) },
  { key: "disk_free_bytes", label: "磁盘剩余", unit: "bytes", source: (item) => Number(item.disk_free_bytes || 0) },
  { key: "disk_inodes_used_pct", label: "inode", unit: "%", source: (item) => Number(item.disk_inodes_used_pct || 0) },
  { key: "disk_read_bps", label: "磁盘读", unit: "B/s", source: (item) => Number(item.disk_read_bps || 0) },
  { key: "disk_write_bps", label: "磁盘写", unit: "B/s", source: (item) => Number(item.disk_write_bps || 0) },
  { key: "disk_read_iops", label: "读 IOPS", unit: "ops/s", source: (item) => Number(item.disk_read_iops || 0) },
  { key: "disk_write_iops", label: "写 IOPS", unit: "ops/s", source: (item) => Number(item.disk_write_iops || 0) },
  { key: "load1", label: "负载", unit: "", source: (item) => Number(item.load1 || 0) },
  { key: "net_rx_bps", label: "网络 RX", unit: "B/s", source: (item) => Number(item.net_rx_bps || 0) },
  { key: "net_tx_bps", label: "网络 TX", unit: "B/s", source: (item) => Number(item.net_tx_bps || 0) },
  { key: "net_rx_packets_ps", label: "收包", unit: "pkt/s", source: (item) => Number(item.net_rx_packets_ps || 0) },
  { key: "net_tx_packets_ps", label: "发包", unit: "pkt/s", source: (item) => Number(item.net_tx_packets_ps || 0) },
];
...
export function formatMetricValue(metricKey: MetricKey, value: number) {
  switch (metricKey) {
    ...
    case "disk_write_iops":
      return `${Math.round(Number(value || 0))} ops/s`;
    case "net_rx_packets_ps":
    case "net_tx_packets_ps":
      return `${Math.round(Number(value || 0))} pkt/s`;
    default:
...
export function mergeLatestHistory(
  ...
  for (const metric of METRICS) {
    ...
    const merged = [...(nextHistory[metric.key] || [])].filter(
      (existing) => new Date(existing.ts).getTime() !== new Date(point.ts).getTime(),
    );
    merged.push(point);
...
export function getWindowPoints(
  ...
  const cutoff = Date.now() - windowSec * 1000;
  return raw
    .map((point) => ({ ts: new Date(point.ts).getTime(), value: Number(point.value || 0) }))
